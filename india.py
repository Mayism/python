...
class Spider():
    ...
    # title_root_pattern = '<div _ngcontent-serverapp-c12 class="topic ng-star-inserted">([\s\S]*?)</div>'
    title_pattern = '<h1 _ngcontent-serverapp-c12 class="topicTitle">([\s\S]*?)</h1>'
    ...
    def __fetch_content(self, url):
        ...
        try:
            ...
            course = doc.find('h1', class_='topicTitle').get_text()
            print(course)
            #关闭浏览器
            driver.quit()
    ...
    def __getUrl(self):
        worksheet = xlrd.open_workbook('D:\india.xls')
    ...
    def go(self):
        ...
        for i in url:
            try:
                ...
                print('博客:' + str(title).split("'")[5])
                sheet.write(row, 0, str(title).split("'")[0])
